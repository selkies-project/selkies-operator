...
RUN go build -o image_puller_worker cmd/image_puller_worker/*.go
...
COPY --from=build /go/src/selkies.io/controller/image_puller_worker /usr/local/bin/image-puller-worker

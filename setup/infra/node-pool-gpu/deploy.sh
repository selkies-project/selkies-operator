...
# Fetch any Secret Manager secrets named broker-${TF_VAR_region}-node-pool-gpu-override-* and same them to *_override.tf files.
for secret in $(gcloud -q secrets list --filter=name~broker-${TF_VAR_region}-node-pool-gpu-override- --format="value(name)"); do
    latest=$(gcloud secrets versions list ${secret} --sort-by=created --format='value(name)' --filter='STATE=enabled' --limit=1)
    [[ -z "${latest}" ]] && log_red "WARN: no enabled versions found for secret ${secret}" && continue
    dest="${secret/broker-${TF_VAR_region}-node-pool-gpu-override-/}_override.tf"
    log_cyan "Creating ${dest} from secret: ${secret}"
    gcloud -q secrets versions access ${latest} --secret ${secret} > ${dest}
done
